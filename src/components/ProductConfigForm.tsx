...
export const ProductConfigForm: React.FC<ProductConfigFormProps> = ({
  ...
  const [resendDiagnosticResult, setResendDiagnosticResult] = useState<any | null>(null);
  const [testEmailRecipient, setTestEmailRecipient] = useState<string>('gestsocialgabriele@gmail.com');
  const [isSendingTestEmail, setIsSendingTestEmail] = useState<boolean>(false);
  const [testEmailFeedback, setTestEmailFeedback] = useState<any | null>(null);

  const handleRunResendDiagnostic = async (testRecipient?: string) => {
    if (testRecipient) {
      setIsSendingTestEmail(true);
    } else {
      setIsDiagnosingResend(true);
      setResendDiagnosticResult(null);
    }
    ...
    try {
      ...
      setResendDiagnosticResult(data);
      if (data.testEmailResult) {
        setTestEmailFeedback(data.testEmailResult);
      }
    } catch (err: any) {
      setResendDiagnosticResult({
        success: false,
        error: err?.message || 'Errore durante la chiamata di diagnostica a Resend',
      });
    } finally {
      setIsDiagnosingResend(false);
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                <button
                  type="button"
                  onClick={() => handleRunResendDiagnostic()}
                  disabled={isDiagnosingResend}
                  className="px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-medium flex items-center gap-1.5 transition-colors cursor-pointer disabled:opacity-50 shadow-sm"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isDiagnosingResend ? 'animate-spin' : ''}`} />
                  <span>{isDiagnosingResend ? 'Verifica in corso...' : '🔍 Diagnostica Domini & Connessione Resend'}</span>
                </button>
                <a
              ...
              {resendDiagnosticResult && (
                <div
                  className={`p-3.5 rounded-xl border text-xs space-y-3 animate-in fade-in duration-200 ${
                    resendDiagnosticResult.success
                      ? 'bg-emerald-50 border-emerald-200 text-emerald-900'
                      : resendDiagnosticResult.diagnosticStatus === 'team_mismatch'
                ...
                >
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <div className="font-bold text-sm">
                          {resendDiagnosticResult.success
                            ? 'Connessione e Dominio Verificati con Successo!'
                            : resendDiagnosticResult.diagnosticStatus === 'team_mismatch'
                            ? 'Attenzione: Team Mismatch Rilevato su Resend'
                            : 'Problema di Configurazione Resend'}
                        </div>
                        <p className="mt-0.5 leading-relaxed text-xs">
                          {resendDiagnosticResult.diagnosisMessage || resendDiagnosticResult.error}
                        </p>
                      </div>
                    </div>
                    <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-white/80 border shrink-0">
                      Chiave: {resendDiagnosticResult.maskedKey || 'N/D'}
                    </span>
                  </div>

                  {/* Domains List from Resend */}
                  {resendDiagnosticResult.domainsList && (
                    <div className="bg-white/80 p-2.5 rounded-lg border border-slate-200/60 space-y-1.5">
                      <div className="text-[11px] font-semibold text-slate-700 flex items-center justify-between">
                        <span>Domini accessibili da questa chiave API su Resend:</span>
                        <span className="font-normal text-slate-500">
                          {resendDiagnosticResult.domainsList.length} trovati
                        </span>
                      </div>
                      {resendDiagnosticResult.domainsList.length > 0 ? (
                        <div className="space-y-1">
                          {resendDiagnosticResult.domainsList.map((d: any, idx: number) => (
                            <div
                              key={idx}
                              className="flex items-center justify-between text-[11px] font-mono bg-slate-50 px-2 py-1 rounded border border-slate-200/50"
                            >
                        ...
                        <div className="text-[11px] text-slate-500 italic">
                          Nessun dominio trovato su questo account Resend per questa chiave.
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {resendDiagnosticResult.actionSteps && resendDiagnosticResult.actionSteps.length > 0 && (
                    <div className="bg-white p-3 rounded-lg border border-amber-200 text-slate-800 space-y-1.5">
                      <div className="font-semibold text-[11px] text-amber-900 uppercase tracking-wider flex items-center gap-1">
                      ...
                      <ol className="list-decimal list-inside space-y-1 text-slate-700 leading-relaxed text-[11px]">
                        {resendDiagnosticResult.actionSteps.map((step: string, sIdx: number) => (
                          <li key={sIdx}>{step}</li>
                        ))}
                      </ol>
                    </div>
                  ...
                  {/* Single Test Email Tool */}
                  <div className="pt-2 border-t border-slate-200/70">
                    <div className="text-[11px] font-semibold text-slate-800 mb-1 flex items-center justify-between">
                      <span>Invia un'email di prova immediata:</span>
                      <span className="text-[10px] text-slate-500">Verifica la trasmissione reale</span>
                    </div>
                    <div className="flex gap-2">
                      <input
                        type="email"
                        placeholder="tuo-indirizzo@email.com"
                        value={testEmailRecipient}
                        onChange={(e) => setTestEmailRecipient(e.target.value)}
                        className="flex-1 px-2.5 py-1.5 bg-white border border-slate-200 rounded-lg text-xs text-slate-900 focus:outline-none focus:border-slate-800"
                      />
                      <button
                        type="button"
                        onClick={() => handleRunResendDiagnostic(testEmailRecipient)}
                        disabled={isSendingTestEmail || !testEmailRecipient.includes('@')}
                        className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-medium flex items-center gap-1 transition-colors cursor-pointer disabled:opacity-50 shrink-0"
                      >
                        <Send className={`w-3 h-3 ${isSendingTestEmail ? 'animate-spin' : ''}`} />
                        <span>{isSendingTestEmail ? 'Invio...' : 'Invia Prova'}</span>
                      </button>
                    </div>

                    {testEmailFeedback && (
                      <div
                        className={`mt-2 p-2 rounded-lg text-[11px] border font-mono ${
                      ...
                      >
                        {testEmailFeedback.success ? (
                          <div>
                            <strong>✅ Email spedita con successo!</strong>
                            <div className="text-[10px] mt-0.5 text-emerald-800">
                              Message ID: {testEmailFeedback.messageId} (Controlla la tua casella di posta)
                            </div>
                          </div>
                        ) : (
                          <div>
                            <strong>❌ Invio fallito:</strong> {testEmailFeedback.error}
                          </div>
                        )}
                      </div>
                    )}
